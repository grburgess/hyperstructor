...
hyperstructor_dev_log_handler = handlers.TimedRotatingFileHandler(
...
hyperstructor_dev_log_handler.setFormatter(_dev_formatter)
hyperstructor_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

hyperstructor_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

hyperstructor_usr_log_handler.setLevel(logging.INFO)
...
hyperstructor_usr_log_handler.setFormatter(_usr_formatter)
...
hyperstructor_console_log_handler = logging.StreamHandler(sys.stdout)
hyperstructor_console_log_handler.setFormatter(_console_formatter)
hyperstructor_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    hyperstructor_usr_log_handler.addFilter(warning_filter)
    hyperstructor_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    hyperstructor_usr_log_handler.removeFilter(warning_filter)
    hyperstructor_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    hyperstructor_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(hyperstructor_dev_log_handler)

    log.addHandler(hyperstructor_console_log_handler)

    log.addHandler(hyperstructor_usr_log_handler)
